...
class _AirPayState extends State<AirPay> {
 ...
 var stringAll = '${widget.user.email ?? ''}${widget.user.fname ?? ''}${widget.user.lname ?? ''}${widget.user.fulladdress ?? ''}${widget.user.city ?? ''}${widget.user.state ?? ''}${widget.user.country ?? ''}${widget.user.amount ?? ''}${widget.user.orderid ?? ''}$formattedDate';
        
        var str2 = '${widget.user.username}~:~${widget.user.password}';
        var key = sha256.convert(utf8.encode(str2));
        var checksum = sha256.convert(utf8.encode('$key@$stringAll'));
    ...
    String productionURL = "https://payments.kmmrcepay.com/pay/index.php"; //https://payments.kmmrcepay.com/
    var isGateWay = productionURL;
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      String errMsg = '';
      if (widget.user == null) {
        errMsg = 'Kindly enter your KMMRCE Pay details to proceed';
      }
      if (widget.user.protoDomain == null || widget.user.protoDomain.isEmpty) {
        errMsg = 'Kindly enter your KMMRCE Pay protoDomain to proceed';
      } else if (widget.user.merchantId == null ||
          widget.user.merchantId.isEmpty) {
        errMsg = 'Kindly enter your KMMRCE Pay MerchantID to proceed';
      } else if (widget.user.secret == null || widget.user.secret.isEmpty) {
        errMsg = 'Kindly enter your KMMRCE Pay secretID to proceed';
      } else if (widget.user.successUrl == null ||
          widget.user.successUrl.isEmpty) {
        errMsg = 'Kindly enter your KMMRCE Pay SuccessURL to proceed';
      }
  ...
  fetchDetails() async {
    ...
    String productionDataURL = "https://payments.kmmrcepay.com/sdk/a.php";
    var isGateWay = productionDataURL;
